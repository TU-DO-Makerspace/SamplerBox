...
class DisplayServer(object):
	...
	def set_layer1(self, val):
		if not self._initialized:
			return
		
		self._layer1_val = val

	def set_layer2(self, val, duration_s):
		if not self._initialized:
			return

		self._layer2_val = val
	...
	def _display(self, val):
		if val == None:
			...
			self._seg1.value = '0'
			self._seg2.value = str(val)
		else:
			self._seg1.value = str(val)[0]
			self._seg2.value = str(val)[1]
